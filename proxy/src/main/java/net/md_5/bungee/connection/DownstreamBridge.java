...
public class DownstreamBridge extends PacketHandler {
	...
	public void handle(ScoreboardObjective objective) throws Exception {
		...
		case 0:
			...
			serverScoreboard.addObjective(new Objective(objective.getName(), objective.getValue(),
					objective.getType() != null ? objective.getType().toString() : null)); // Travertine - 1.7 protocol
																							// support
			break;
		case 1:
	...
	public void handle(PluginMessage pluginMessage) throws Exception {
		...
		if (pluginMessage.getTag()
			...
			if (con.getPendingConnection().getProtocol().newerOrEqual(Protocol.MC_1_8_0)) {
				...
				pluginMessage.setData(
						(bungee.getName() + " (" + bungee.getVersion() + ")" + " <- " + serverBrand).getBytes("UTF-8"));
			}
	...
	public void handle(Kick kick) throws Exception {
		...
		ServerKickEvent event = bungee.getPluginManager()
				.callEvent(new ServerKickEvent(con, server.getInfo(), cs, def, ServerKickEvent.State.CONNECTED));
	...
	public void handle(TabCompleteResponse tabCompleteResponse) throws Exception {
		...
		if (commands == null) {
			commands = Lists.transform(tabCompleteResponse.getSuggestions().getList(), (Suggestion input) -> input.getText());
		}
			...
			if (!commands.equals(tabCompleteResponseEvent.getSuggestions())) {
				if (tabCompleteResponse.getCommands() != null) {
					...
					tabCompleteResponse.setSuggestions(
						new Suggestions(
							range,
							Lists.transform(tabCompleteResponseEvent.getSuggestions(), input -> new Suggestion(range, input))
						)
					);
				}
			}
