package net.md_5.bungee.protocol.packet;

import java.util.UUID;

import io.netty.buffer.ByteBuf;
import lombok.Data;
import lombok.EqualsAndHashCode;
import lombok.NoArgsConstructor;
import net.md_5.bungee.protocol.AbstractPacketHandler;
import net.md_5.bungee.protocol.Packet;
import net.md_5.bungee.protocol.Direction;
import net.md_5.bungee.protocol.ProtocolVersion;

@Data
@NoArgsConstructor
@EqualsAndHashCode(callSuper = false)
public class PlayerListItem extends Packet
{

    private Action action;
    private Item[] items;

    @Override
    public void read(ByteBuf buf, Direction direction, ProtocolVersion protocolVersion)
    {
        if ( protocolVersion.olderThan(ProtocolVersion.MC_1_8_0 ))
        {
            items = new Item[ 1 ];
            Item item = items[ 0 ] = new Item();
            item.displayName = item.username = readString( buf, protocolVersion );
            action = !buf.readBoolean() ? Action.REMOVE_PLAYER : Action.ADD_PLAYER;
            item.ping = buf.readShort();
        } else
        {
            action = Action.values()[ Packet.readVarInt( buf )];
            items = new Item[ Packet.readVarInt( buf ) ];
            for ( int i = 0; i < items.length; i++ )
            {
                Item item = items[ i ] = new Item();
                item.setUuid( Packet.readUUID( buf ) );
                switch ( action )
                {
                    case ADD_PLAYER:
                        item.username = Packet.readString( buf );
                        item.properties = new String[ Packet.readVarInt( buf ) ][];
                        for ( int j = 0; j < item.properties.length; j++ )
                        {
                            String name = Packet.readString( buf );
                            String value = Packet.readString( buf );
                            if ( buf.readBoolean() )
                            {
                                item.properties[j] = new String[]
                                {
                                    name, value, Packet.readString( buf )
                                };
                            } else
                            {
                                item.properties[j] = new String[]
                                {
                                    name, value
                                };
                            }
                        }
                        item.gamemode = Packet.readVarInt( buf );
                        item.ping = Packet.readVarInt( buf );
                        if ( buf.readBoolean() )
                        {
                            item.displayName = Packet.readString( buf );
                        }
                        break;
                    case UPDATE_GAMEMODE:
                        item.gamemode = Packet.readVarInt( buf );
                        break;
                    case UPDATE_LATENCY:
                        item.ping = Packet.readVarInt( buf );
                        break;
                    case UPDATE_DISPLAY_NAME:
                        if ( buf.readBoolean() )
                        {
                            item.displayName = Packet.readString( buf );
                        }
                }
            }
        }
    }

    @Override
    public void write(ByteBuf buf, Direction direction, ProtocolVersion protocolVersion)
    {
        if ( protocolVersion.olderThan(ProtocolVersion.MC_1_8_0) )
        {
            Item item = items[0]; // Only one at a time
            writeString( item.displayName, buf, protocolVersion ); // TODO: Server unique only!
            buf.writeBoolean( action != Action.REMOVE_PLAYER );
            buf.writeShort( item.ping );
        } else
        {
            Packet.writeVarInt( action.ordinal(), buf );
            Packet.writeVarInt( items.length, buf );
            for ( Item item : items )
            {
                Packet.writeUUID( item.uuid, buf );
                switch ( action )
                {
                    case ADD_PLAYER:
                        Packet.writeString( item.username, buf );
                        Packet.writeVarInt( item.properties.length, buf );
                        for ( String[] prop : item.properties )
                        {
                            Packet.writeString( prop[ 0], buf );
                            Packet.writeString( prop[ 1], buf );
                            if ( prop.length >= 3 )
                            {
                                buf.writeBoolean( true );
                                Packet.writeString( prop[ 2], buf );
                            } else
                            {
                                buf.writeBoolean( false );
                            }
                        }
                        Packet.writeVarInt( item.gamemode, buf );
                        Packet.writeVarInt( item.ping, buf );
                        buf.writeBoolean( item.displayName != null );
                        if ( item.displayName != null )
                        {
                            Packet.writeString( item.displayName, buf );
                        }
                        break;
                    case UPDATE_GAMEMODE:
                        Packet.writeVarInt( item.gamemode, buf );
                        break;
                    case UPDATE_LATENCY:
                        Packet.writeVarInt( item.ping, buf );
                        break;
                    case UPDATE_DISPLAY_NAME:
                        buf.writeBoolean( item.displayName != null );
                        if ( item.displayName != null )
                        {
                            Packet.writeString( item.displayName, buf );
                        }
                        break;
                }
            }
        }
    }

    @Override
    public void handle(AbstractPacketHandler handler) throws Exception
    {
        handler.handle( this );
    }

    public static enum Action
    {

        ADD_PLAYER,
        UPDATE_GAMEMODE,
        UPDATE_LATENCY,
        UPDATE_DISPLAY_NAME,
        REMOVE_PLAYER;
    }

    @Data
    public static class Item
    {

        // ALL
        private UUID uuid;

        // ADD_PLAYER
        private String username;
        private String[][] properties;

        // ADD_PLAYER & UPDATE_GAMEMODE
        private int gamemode;

        // ADD_PLAYER & UPDATE_LATENCY
        private int ping;

        // ADD_PLAYER & UPDATE_DISPLAY_NAME
        private String displayName;

    }
}
